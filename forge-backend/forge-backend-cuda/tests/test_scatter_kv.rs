...
fn scatter_kv_matches_host_loop() {
    ...
    let fresh_pool = || {
        backend
            .allocate_zeros(&shape, DType::F32)
            .expect("pool alloc")
    };
    ...
    let src4_t = backend
        .copy_from_host_f32(&src4, &[4, 3])
        .expect("src4 upload");
    let slots4 = [0i32, 1, 2, 3];

    let mut pool_ref2 = fresh_pool();
    backend
        .paged_write_kv(&mut pool_ref2, &src4_t, &slots4)
        .unwrap();
    backend.synchronize().unwrap();
    ...
    assert_eq!(
        got2, expected2,
        "contiguous scatter_kv must match host loop"
    );
    ...
    assert_eq!(
        got_zero,
        vec![0.0_f32; 24],
        "n_rows=0 leaves pool untouched"
    );

    // ── Case 5: F16 parity ───────────────────────────────────────────
    let src_f16_data: Vec<half::f16> = (0..9)
    ...
    backend
        .paged_write_kv(&mut pool_f16_ref, &src_f16, &slots)
        .unwrap();
    backend.synchronize().unwrap();
    let exp_f16 = backend
        .copy_to_host_f32(&backend.cast(&pool_f16_ref, DType::F32).unwrap())
        .unwrap();

    let mut pool_f16_scatter = backend.allocate_zeros(&shape, DType::F16).unwrap();
    let nf = backend.stage_slot_mapping(&slots).unwrap();
    backend
        .scatter_kv(&mut pool_f16_scatter, &src_f16, nf)
        .unwrap();
    backend.synchronize().unwrap();
    let got_f16 = backend
        .copy_to_host_f32(&backend.cast(&pool_f16_scatter, DType::F32).unwrap())
        .unwrap();
